...
roteia para as skills `criar-*` e os reviewers certos. Análogo ao `revisar-mudancas`, mas na
...
**Argumento:** descrição da feature ou nome da entidade (ex.: `/planejar-feature subject`,
`/planejar-feature alocar professor em turma`). Sem argumento, pergunte o que vamos planejar
antes de seguir.
...
- **Entidade folha vs cruza ator** — toca outro ator (`users`/`staff`/`students`/`guardians`)?
  Se sim, o controller precisa de peso hierárquico → forma completa (ver `criar-controller`).
- **Frontend local vs global** — estado só na tela (hooks, padrão User) ou entidade cross-app
  (redux-sagas, padrão Student)? Cruze com o modelo de dados (ver `criar-pagina`).
...
|---|---|---|---|
| 1 | `criar-model` | (via migration/controller) | sim se schema novo |
| 2 | `criar-migration` | `migration-review` | sim se schema novo |
| 3 | `criar-controller` | `controller-review` (+ `backend-auth-review`) | sim se expõe a entidade |
| 4 | `criar-rota` | `backend-auth-review` | sim se há HTTP |
| 5 | `criar-pagina` | `ui-kit-review` | sim se há UI |

### 4. Marcar os pontos human-in-the-loop
Conforme `.claude/context/governanca.md`: migration destrutiva, auth/peso hierárquico, exclusão
de dados, schema core. Liste explicitamente onde a feature vai me pedir aprovação.

### 5. Levantar os gates
Conforme `.claude/context/governanca.md` → seção "Gates reais". Diga quais valem para esta
...
> (cada passo aponta seu reviewer), e no fim `revisar-mudancas` → `descrever-pr` → `sugerir-commits`
> fecham a entrega.
