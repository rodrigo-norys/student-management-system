...
export default function StaffForm() {
  ...
  const mainPhoto = getStaffImageUrl(form.avatar_url);
  ...
  useEffect(() => {
    ...
    const addressList =
      staffMember.addresses?.length > 0
        ? [...staffMember.addresses].sort((a, b) => (a.id || 0) - (b.id || 0))
        : [{ ...EMPTY_ADDRESS }];
    ...
    setForm({
      ...
      cpf: staffMember.cpf || '',
      phone: maskPhone(staffMember.phone || ''),
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newErrors = {};
    const { full_name, email, cpf, phone, job_title, birth_date, hiring_date } =
      form;

    if (full_name.length < 3 || full_name.length > 150)
      newErrors.full_name = 'Invalid name length';
    if (!isEmail(email)) newErrors.email = 'Invalid email';
    if (!cpfValidator.isValid(cpf)) newErrors.cpf = 'Invalid CPF';
    if (phone.replace(/\D/g, '').length < 10) newErrors.phone = 'Invalid phone';
    if (!job_title) newErrors.job_title = 'Required';
    if (!birth_date) newErrors.birth_date = 'Required';
    if (!hiring_date) newErrors.hiring_date = 'Required';
    ...
    const dataToSubmit = {
      ...
      shouldStay: stayOnPage,
    };

    dispatch(
      id
        ? actions.updateStaffRequest({ id, ...dataToSubmit })
        : actions.createStaffRequest(dataToSubmit),
    );
  };
  ...
  return (
    <PageContainer>
      ...
      <Styled.HeaderContent>
        ...
        {id && (
          <Styled.PrimaryButton as={Link} to={`/staff/${id}`}>
